...
// ---------------------------------------------------------
  ...
  void AndroidBluetoothServer::handleWifiInfoRequest(QByteArray &buffer, uint16_t length) {
    ...
    const QString ssid = configuration_->getSettingByName<QString>("Wireless", "HotspotSSID");
    const QString password = configuration_->getSettingByName<QString>("Wireless", "HotspotPassword");
